...
\newgeometry{margin=1.5cm, top=1.2cm,right=1.5cm,left=1.5cm,bottom = 1.5cm, footskip = 1 cm, headheight=20pt,headsep=0.2cm}
\resetHeadWidth
...
\begin{itemize}
\item 
При $\pi/2$ и $3\pi/2$ функция меняется на кофункцию. %(Детский жест на подсказку. Вопрос. Меняем на конфункцию? точки на вертикали, значит головой киваем сверху вниз. Получается согласие \glqqДа\grqq "--- меняем на конфункцию).
\item
При $\pi$ и $2\pi$ функция не меняется на кофункцию. %(Аналогично, точки по горизонтали, значит машем головой в горизонтальной плоскости, получается отказ \glqqНет\grqq "--- не меняем).
\end{itemize}
\end{enumerate}
...
\newgeometry{margin=1.6cm, top=1.2cm,right=1.5cm,left=1.5cm,bottom = 0.8cm, footskip = 0.5 cm, headheight=20pt,headsep=0.2cm}
\resetHeadWidth
\newpage
