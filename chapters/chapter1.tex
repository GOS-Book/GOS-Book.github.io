...
\item 
$\forall a\in \bbR$, $a\ne 0,\quad \exists \frac{1}{a} \in \bbR\cquad a\cdot \frac{1}{a} = 1$, $\frac{1}{a}$ называется \textit{обратным} числом для $a$ (\textit{существование обратного}).
\end{enumerate}
\item
...
$$
\forall n \in \bbN \quad x_n \le x_{n+1} \quad (x_n \ge x_{n+1}).
$$
Последовательность $\{x_n\}$ называется \textit{строго возрастающей \textup{(}убывающей\textup{)}}, если 
$$
\forall n \in \bbN \quad x_n < x_{n+1} \quad (x_n > x_{n+1}).
$$
\end{defn}
