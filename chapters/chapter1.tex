...
\InsertBoxL{0}{\llap{\includegraphics[height=2.5cm]{Bang}}}%
\mbox{\kern0.1em}
...
\item
$a\le b,\ b\le c \Rightarrow a\le c\quad \forall a,b,c\in \bbR$.
\end{enumerate}
\item
...
$$
Тогда $\exists c \in \bbR$ такое, что
$$
...
Определим теперь отношения порядка <<$<$>>, <<$>$>>, <<$\ge$>> и операции вычитания~<<$-$>> и деления~<<$/$>> на множестве действительных чисел:
\begin{itemize}[noitemsep]
\item 
$a < b \Leftrightarrow a \le b \text{ и } a\neq b$;
\item
...
\item
Множество \textit{рациональных} чисел $\bbQ=\{\,x\,\big|\,x=\frac{p}{q},\ q\in\bbN,\ p\in\bbZ\,\}$.
\item 
...
\item 
Наряду с числовой прямой определим \textit{расширенную числовую прямую:} множество $\bboR=\bbR\cup \{-\infty, {+\infty}\}$, в котором определены отношения порядка <<$\le$>>, операции <<$+$>>, <<$\cdot$>> для элементов из $\bbR$. Элементы $-\infty$, $+\infty$ не содержатся в $\bbR$, для них определены отношения порядка: $\forall x \in \bbR$\quad$-\infty < x < +\infty$. Кроме того, частично определены операции <<$+$>>, <<$-$>>, <<$\cdot$>> и <<$/$>>
\begin{align*}
&+\infty + a = +\infty,\quad \forall a \in \bbR\cup\{+\infty\};\quad -\infty+ a = -\infty\quad \forall a \in \bbR\cup\{-\infty\};\\
&-\infty - a = -\infty,\quad \forall a \in \bbR\cup\{-\infty\};\quad +\infty - a = +\infty\quad \forall a \in \bbR\cup\{+\infty\};\\
&\forall a > 0, a\in\bbR\cup\{+\infty\}\quad +\infty\cdot a = +\infty\quad\text{и}\quad -\infty\cdot a = -\infty;\\ 
&\forall a < 0,  a\in\bbR\cup\{-\infty\}\quad +\infty\cdot a = -\infty\quad\text{и}\quad -\infty\cdot a = +\infty;\\
& \frac{a}{-\infty} = \frac{a}{+\infty} = 0 \quad \forall a \in \bbR;
\end{align*}
Но не определены: сумма $+\infty + (-\infty)$, произведение $0\cdot (\pm \infty)$, частное $\pm \infty / \pm \infty$.
\item
Промежутки на $\bbR$ (на $\bboR$)
\begin{itemize}[nolistsep, label = $\scriptstyle\blacktriangleright$]
\item 
...
\begin{notion}
Множество $\bbQ$ рациональных чисел удовлетворяет аксиомам~1--6, но не удовлетворяет аксиоме 15. Покажем последнее. Пусть $A = \{\,a\,\big|\,a\in\bbQ,\ a>0,\ a^2<2\,\}$, $B = \{\,b\,\big|\,b\in\bbQ,\ b>0,\ b^2>2\,\}$. Тогда во множестве $\bbQ$ не существует числа $c$ ($c\in\bbQ$) со свойством: $a\le c\le b \quad \forall a \in A,\ \forall b\in B$.
\end{notion}
...
\end{lemm}
\begin{proof}
Пусть $\{x_n\}$ фундаментальна, т.е. удовлетворяет \eqref{eq:ch1:fundamen}. Положим $\epsilon = 1$, $m = N_1$. Тогда $\forall n > N_1\colon |x_n-x_{N_1}| <\epsilon = 1 \Leftrightarrow \forall n > N_1\colon |x_n - x_{N_1}| < 1$, т.е. $x_{N_1} - 1< x_n < x_{N_1}+1$. Если теперь через $a$ и $b$ обозначим наименьшее и наибольшее из чисел $x_1$, $\dots$, $x_{N_1}$, $x_{N_1}-1$, $x_{N_1}+1$, то, очевидно, $a \le x_n \le b$, $\forall n$.
