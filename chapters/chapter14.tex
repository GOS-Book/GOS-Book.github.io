\part[Кратные интегралы и теория поля]{Кратные интегралы и теория поля}%\protect\linebreak \vbox{\vspace*{5\baselineskip}}\centerline{\includegraphics[height=0.3\textheight]{pictures/Foxi/4}}}
...
Пусть в плоскости $\bbR^2$ фиксирована некоторая прямоугольная система координат $x,y$, и пусть $\Delta$ "--- некоторая открытая клетка, т.е. $\Delta = (a;b)\times(c;d)$, а $\overline{\Delta}$ "--- её замыкание, т.е. $\overline{\Delta}=[a;b]\times[c;d]$. Через $\partial \Delta$, как обычно, обозначим границу множества $\Delta$.
...
\end{equation}
где обход $\partial\Delta$ совершается против часовой стрелки, если система координат $x,y$ правая (рис 14.1), и по часовой стрелке, если система координат $x,y$ левая (рис 14.2).
\end{lemm}
