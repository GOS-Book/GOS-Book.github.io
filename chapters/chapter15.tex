...
\begin{thm}
Если непрерывно-дифференцируемое векторное поле $\vv{a}(M), \; M \in G$, имеет в области $G$ векторный потенциал $\vv{b}$, то для любой замкнутой кусочно-гладкой поверхности $S: \; S\subset G$ выполняется равенство: $\iint\limits_S \vv{a}\,d\vv{s} = 0$. (Область $\Omega$, ограниченная поверхностью $S$, может не лежать в $G$, хотя $S \subset G$.)
\end{thm}
...
$\iint\limits_S \vv{a}\,d\vv{s} = \iint\limits_{S_1} \vv{a}\,d\vv{s} + \iint\limits_{S_2} \vv{a}\,d\vv{s} = \iint\limits_{S_1} \rot \: \vv{b}\,d\vv{s} + \iint\limits_{S_2} \rot \: \vv{b}\,d\vv{s} \myeq{\textit{Стокс}} \iint\limits_{\gamma_1} \vv{b}\,d\vv{r} + \iint\limits_{\gamma_2} \vv{b}\,d\vv{r} = \iint\limits_{\gamma_1} \vv{b}\,d\vv{r} - \iint\limits_{\gamma_1} \vv{b}\,d\vv{r} = 0$
\end{proof}
