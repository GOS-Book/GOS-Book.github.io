...
\begin{defn}
Пусть $a \in \bbC$ "--- изолированная особая точка регулярной функции $f\colon \overset{\circ}{B}_\rho(a) \to \bbC$, $\rho > 0$. Пусть $\gamma_r \triangleq \{\, z : |z - a| = r\,\}$ "--- положительно ориентированная окружность, причем $0 < r < \rho$. Тогда \textit{вычетом\rindex{вычет} функции $f$ в точке $a$} называется число
