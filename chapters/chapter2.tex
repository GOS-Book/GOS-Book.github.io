...
\end{equation*}
\end{defn}
Если точка $x_0$ -- не предельная для множества $D_f$, то функция $f$ не может иметь предела по Коши в точке $x_0$ (ведь тогда $\forall\, \epsilon$ может $\exists\, \delta$ такая, что $\dneio{\delta}{x_0}\cap D_f=\emptyset$) 
