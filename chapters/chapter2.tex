...
\begin{defn}
Пусть $D_f$ и $Y$ "--- два произвольных множества, и задано некоторое правило $f$, которое каждому элементу $x\in D_f$ ставит в соответствие один и только один некоторый элемент $y=f(x)$ из $Y$. Тогда множество всевозможных пар $(x, f(x))$, $x\in D_f$, называется \textit{функцией}\rindex{функция} и обозначается либо просто $f$, либо $f(x),$ $x\in D_f$, либо $y=f(x),\ x\in D_f$, либо, например, $f\colon D_f \to Y$.
\end{defn}

Функция называется \textit{числовой}, если ее значениями являются действительные числа: $Y \subset \bbR$.
\begin{itemize}[wide, labelwidth=!, labelindent=0pt]
\item
Элемент $x \in D_f$ называется \textit{аргументом}\rindex{аргумент}, или \textit{независимым перемененным}, элемент $y=f(x) \in Y$ "--- \textit{значением функции}, или \textit{зависимым переменным}. Таким образом, $f(x)$ может обозначать как значении функции $f$ на элементе $x$, так и саму функцию $f$.
\item
Множество $D_f$ называется \textit{областью определения} функции $f$. В таком случае, будем говорить, что функция определена на $X$, если $X \subset D_f$.
\item
Множество всех $y=f(x)$, $x\in D_f$ называется \textit{множеством значений} функции $f$ и обозначается $f(D_f)$ 
$$
...
\begin{defn}
Пусть функция $f$ определена на множестве $X$. Тогда 
$$
...
\begin{defn}
Точка $c \in \bbR$ называется \textit{предельной точкой множества}\rindex{точка множества!предельная} $G$, если $$\forall \epsilon >0 \quad \dneio{\epsilon}{c} \cap G \ne \emptyset.$$
\end{defn}
...
Пусть точка $x_0$ является предельной для областей определения функций $f(x)$, $g(x)$ и $h(x)$. Тогда если существуют пределы $\lim_{x \to x_0}\limits f(x) = \lim_{x \to x_0}\limits h(x) = A \in \bboR$, причем
$$
\exists \delta > 0\cquad \forall x \in \dneio{\delta}{x_0}\quad f(x)\le g(x) \le h(x),
$$
то и $\lim_{x \to x_0}\limits g(x) = A.$
\end{thm}
...
\begin{thm}[Больцано"--~Вейерштрасса](доказана в билете \textnumero 1)
