...
\begin{enumerate}
\item
\textit{проколотой $\epsilon$-окрестностью числа} $c$ называется множество $\dneio{\epsilon}{c} \triangleq O_\epsilon(c)\setminus\{c\}=(c-\epsilon;c)\cup(c;c+\epsilon)$;
\item
\textit{проколотой окрестностью точки} $c$ называется $\dnei{c} \triangleq O(c)\setminus \{c\}$;
\end{enumerate}
\quad\textbullet\quad А если $c$ -- бесконечно удаленная точка, то, по определению, $\dneio{\epsilon}{c}= O (c) $.
\end{defn}
