...
Обозначется $y=f(x),\ x\in D_f$, или, например, $f: D_f \to \bbR$. Тогда
\begin{itemize}
\item
...
\begin{defn}[Определение предела функции по Коши\rindex{предел!функции по Коши}]\label{exp12} 
...
\begin{defn}[Определение предела функции по Гейне\rindex{предел!функции по Гейне}]\label{exp11} 
...
\begin{thm}[Больцано-Вейерштрасса]\textup{(}доказана в билете №1\textup{)}
У любой ограниченной последовательности существует сходящаяся подпоследовательность.
\end{thm}


\begin{thm} [Вейерштрасса\rindex{теорема!Вейерштрасса}] \label{ch1n1}
