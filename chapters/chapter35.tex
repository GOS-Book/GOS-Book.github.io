...
\begin{thm}[о единственности разложения в ряд Лорана] \label{2}
...
Из следствия 1 и теоремы \ref{2} получаем
...
В определении 3 точка $a$ может быть как конечной точкой (тогда $\overset{\circ}{B}_\rho(a) = \{ z \: \big| \: 0 < |z - a| < \rho\}$), так и бесконечной (тогда $\overset{\circ}{B}_\rho(\infty) = \{ z \: \big| \: |z| > \rho\}$).
...
\begin{proof}
Очевидно следует из только что доказанной теоремы 5.
\end{proof}
...
Тогда говорят, что функция $g$ имеет в точке $a$ \textit{нуль $m$-го порядка (или $m$-й кратности)}. Если же $g(a) \not= 0$, то говорят, что точка $a$ не является нулем функции $g$ (или для следствия 5: функция $g$ имеет в точке $a$ нуль нулевого порядка).
\end{defn}
