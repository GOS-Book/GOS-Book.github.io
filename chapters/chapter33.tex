\part[Теория функций комплексного переменного]{Теория функций комплексного переменного}%\protect\linebreak \vbox{\vspace*{5\baselineskip}}\centerline{\includegraphics[height=0.3\textheight]{pictures/Foxi/10}}}
...
\section[Дифференцируемость функций комплексного переменного]{Дифференцируемость функций комплексного переменного\footnote{Здесь и далее тонкая фиолетовая линия слева от текста обозначает материал, который Половинкин Е.С. на своей консультации выделил как достаточный для рассказа билета.}}
...
$$
\forall \epsilon > 0\quad \exists N(\epsilon)\colon \forall n \ge N(\epsilon) \quad z_n \in B_\epsilon(A)\quad (\text{т.е. }|z_n-A|<\epsilon).
$$
\end{defn}
\begin{defn}\label{ch33.defn3}
Последовательность $\{z_n\}$ \textit{сходится (стремится) к бесконечности} $(z_n\to\infty)$, если 
$$
\forall \epsilon > 0\quad \exists N(\epsilon)\colon \forall n \ge N(\epsilon) \quad |z_n|>\epsilon.
$$
\end{defn} 
...
$$
\forall \epsilon > 0 \exists \delta = \delta(\epsilon)>0:\forall z \in \overset{\circ} {B}_\delta(z_0)\cap G \quad f(z) \in B_\epsilon(A). 
$$ 
\end{defn}

\begin{thm} [о трех функциях из матанализа для $\bbR$.]
\label{exp14}
Пусть $x_0$ "--- предельная точка множества $D\subset \bbR$. Пусть $D = D_f=D_g=D_h$ и $\exists \delta > 0$ $\forall x \in D \cap \dneio{\delta}{x_0}:$
$$f(x)\le g(x) \le h(x),$$ тогда если $\exists \lim_{x \to x_0}\limits f(x) = A \in \bbR$ , $\exists \lim_{x \to x_0}\limits h(x) = A$, тогда и $\exists \lim_{x \to x_0}\limits g(x) = A$.
\end{thm}
...
\end{defn}
\end{leftbar}
Пусть $\Delta z = z - z_0$ и $\Delta f = f(z) - f(z_0)$. Определение \ref{exp5} означает, что $\forall \epsilon > 0 \exists \delta=\delta(\epsilon)>0$ такое, что для $\forall \Delta z:\quad 0 <|\Delta z| <\delta$ справедливо 
\begin{equation}
\label{exp7}
...
\begin{equation}
\label{exp4}
\Delta f = A\cdot \Delta z + \alpha(\Delta z), \forall \Delta z: 0 < |\Delta z|< r,
\end{equation}
...
\end{proof}
\end{leftbar}%\addmppict[0.8]{pictures/ch33pict3}{0.4}{
